...
function LayoutList({layouts}) {
  return View({d: 'flex', flexDirection: 'column', gap: 'sm'}, layouts.map(layout => ([
    View({onClick: openModal(`layout-${layout.id}`), p: 'sm', border: true, borderColor: 'base-400', bgColor: 'base-200'}, layout.name),
    createModal({
      $data: layout,
      mode: 'edit',
