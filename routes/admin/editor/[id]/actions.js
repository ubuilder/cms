...
export async function create_component({ ctx, body, params }) {
  const { id, parent_id = params.id } = body;
...
export async function add_instance({ ctx, body, params }) {
  ...
  const parentInstance = await ctx
    ...
    console.log({parentId, parentInstance})
...
export async function remove_instance({ ctx, params, body }) {
  ...
  const parentInstance = await ctx.table('instances').get({where: {id: parentId || params.id}})
  parentInstance.slot_ids = parentInstance.slot_ids.filter(id => id !== instanceId);
  
  if(instance.id === params.id) return;
